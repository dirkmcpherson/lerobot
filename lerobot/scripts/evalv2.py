...
def main(
    ...
    with torch.no_grad(), torch.autocast(device_type=device.type) if hydra_cfg.use_amp else nullcontext():
        info = eval_policy(
            ...
            start_seed=random.randint(0, 2**30),
    ...
    repo_id = f'js/{pretrained_policy_path.parts[-1]}'
    ...
    for step in range(n_steps):
        frame = {}
        for k,v in d.items():
            if k in features.keys(): frame[k] = v[step]
        dataset.add_frame(frame)
        if d['episode_index'][step] != current_episode:
            current_episode = d['episode_index'][step].item()
